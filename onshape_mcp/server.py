...
# session (Codex reads the first 512 chars as server-wide guidance),
# giving the agent a categorized tool index + gotchas up front so the
# first turns aren't spent guessing tool names.
_INSTRUCTIONS = """\
Codex Onshape MCP — drive real Onshape CAD (documents, part studios,
assemblies, FeatureScript). Tools are lazy-loaded (fetch schemas via
ToolSearch before calling). After every mutation, run
`describe_part_studio` to verify: it returns topology + multi-view
renders in one call (text catches arithmetic/counting errors, images
catch orientation/topology errors). Units: bare numbers = mm, or pass
explicit unit strings like "30 mm". Default sketch planes: Front (XZ,
normal -Y), Top (XY), Right (YZ). NEVER pick faces/edges/mates without
calling `list_entities` or `describe_part_studio` first. If a feature
returns status ERROR, fix or delete it before adding more features.
...
app = Server("codex-onshape-mcp", instructions=_INSTRUCTIONS)
...
def main():
    ...
    if "--sse" in sys.argv or os.getenv("MCP_TRANSPORT") == "sse":
        ...
        print(f"Starting Codex Onshape MCP server in SSE mode on port {port}", file=sys.stderr)
        if reload:
